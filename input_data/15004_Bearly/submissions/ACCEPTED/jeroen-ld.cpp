...
bool circleCircleIntersection( circle c0, circle c1, point &p3, point &p4) {
	point p0(c0.x, c0.y), p1(c1.x, c1.y);
	ld d = dist(p0,p1), r0 = c0.r, r1 = c1.r;
	if( (d > r0 + r1) or (d < fabs(r0 - r1)) ) return false;
	ld a = (r0 * r0 - r1 * r1 + d * d) / (2 * d);
	ld h = sqrt(r0 * r0 - a * a);
...
bool circleLineSegmentIntersection( point E, point L, circle c0, ld &t1, ld &t2) {
	point C(c0.x, c0.y);
	ld r = c0.r;
...
int main() {
	...
	//Graph data
	gw.assign(ps.size(), vector<ld> (ps.size(),0.0));
	...
	else cout << fixed << setprecision(numeric_limits<ld>::digits10 + 1) << gdist[1] << endl;
	return 0;
}
