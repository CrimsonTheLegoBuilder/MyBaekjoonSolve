...
// this is an epsilon used in line_covered
// not sure how much is enough
const ld EPSILONE = 1e-10;
...
bool line_covered(const line l1, const vector<circle> &circles) {
  ...
  // The computed intersection points may actually be
  // slightly off. So we don't fully trust the open/closed
  // parens method here. Instead, we measure how much is
  // covered.
  int depth = 0;
  bool on_line = false;
  ld last_open = numeric_limits<ld>::min();
  ld measure = 0;
  for (const auto s : scope) {
    if (on_line && depth == 0 && s.second == -1)
      last_open = s.first;

    if (on_line && depth == 1 && s.second == 1)
      measure += (s.first - last_open);
  ...
  // if measure is stricly larger than 1, it's definitely covered
  // but it may be a bit less then 1 due to rounding errors
  return (1 - measure) < EPSILONE;
}
