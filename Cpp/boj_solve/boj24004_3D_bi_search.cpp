...
bool check(const Circle& A, const Circle& B, const Circle& C) {
	...
	//if (AB.size() > 1 && ccw(A.c, B.c, b2) < 0) std::swap(b1, b2);
	Pos c1 = A.p(AC[0]), c2 = Pos();
	if (AC.size() > 1) c2 = A.p(AC[1]);
	//if (AC.size() > 1 && ccw(A.c, C.c, c2) < 0) std::swap(c1, c2);
...
//	Pos3D operator + (const Pos3D& p) const { return { x + p.x, y + p.y, z + p.z }; }
//	Pos3D operator - (const Pos3D& p) const { return { x - p.x, y - p.y, z - p.z }; }
//	Pos3D& operator *= (const int& scalar) { x *= scalar; y *= scalar; z *= scalar; return *this; }
...
//		ld m = (s + e) * .5;
//		//std::cout << "r:: " << m << " check:: " << check(a, b, c, m, seq) << "\n";
//		if (check(a, b, c, m, seq)) e = m;
//		else s = m;
//	}
//	return (s + e) * .5;
...
//	std::sort(vd.begin(), vd.end());
//	ld d1 = vd[0], d2 = vd[1], d3 = vd[2];
//	//ll l1 = (q1 - q2).Euc(), l2 = (q2 - q3).Euc(), l3 = (q3 - q1).Euc();
//	//Vll vd = { l1, l2, l3 };
//	//std::sort(vd.begin(), vd.end());
//	//ld d1 = sqrtl(vd[0]), d2 = sqrtl(vd[1]), d3 = sqrtl(vd[2]);
//	if (eq(d3, (d1 + d2))) { std::cout << "Case #" << t << ": " << d3 / 6 / SCALE << "\n"; return; }
//	ld th = cos_2nd(d1, d3, d2);
//	//std::cout << "DEBUG:: d1 = " << d1 << " d2 = " << d2 << " d3 = " << d3 << "\n";
//	Pos a = O, b = Pos(d1, 0).rot(th), c = Pos(d3, 0);
//	//std::cout << "DEBUG:: a = " << a << " b = " << b << " c = " << c << "\n";
//	ld r1 = bi_search(a, b, c, TOL, d3, 1);
//	//r1 = std::min(r1, bi_search(b, c, a, TOL, d3, 1));
//	//r1 = std::min(r1, bi_search(c, a, b, TOL, d3, 1));
//	//std::cout << "r1:: " << r1 << "\n";
//	ld r2 = bi_search(a, b, c, TOL, d3, 2);
//	//r2 = std::min(r2, bi_search(b, c, a, TOL, d3, 2));
//	//r2 = std::min(r2, bi_search(c, a, b, TOL, d3, 2));
//	//std::cout << "r2:: " << r2 << "\n";
//	std::cout << "Case #" << t << ": "<< std::min(r1, r2) / SCALE << "\n";
//	return;
//}
...
//	std::cout << std::fixed;
//	std::cout.precision(15);
//	//std::cout << cos_2nd(sqrtl(2), 0.369819407659848821, 0.369819407659848821 * 3) * 180 / PI << "\n";
//	std::cin >> T;
//	for (int t = 1; t <= T; t++) query(t);
//	return;
//}
