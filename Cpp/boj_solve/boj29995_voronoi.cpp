...
ld voronoi_diagram(const std::vector<Pii>& B, std::vector<Pii> C) {
	int sz = C.size();
	poly.resize(sz);
	for (int i = 0; i < sz; i++) poly[i] = P(C[i]);
	assert(sz);
	ld ret = 0;
	if (sz == 1) {
		for (const Pii& p : B) ret = std::max(ret, (p - C[0]).mag());
		return ret;
	}
	else if (sz == 2) {
		seed[0].push_back(1);
	...
	for (int i = 0; i < N; i++) {
		...
		HP.push_back(L(P(B[0]), P(B[1])));
		HP.push_back(L(P(B[1]), P(B[2])));
		HP.push_back(L(P(B[2]), P(B[3])));
		HP.push_back(L(P(B[3]), P(B[0])));
		std::vector<Pdd> HPI = {};
		if (!half_plane_intersection(HP, HPI)) continue;
		//vd[i] = HPI;
		for (int j = 0; j < HPI.size(); j++) {
...
void solve() {
	...
	std::vector<Pii> box(4);
	for (Pii& p : box) std::cin >> p;
	box = graham_scan(box);
	std::cin >> N;
	C.resize(N);
	for (int i = 0; i < N; i++) std::cin >> C[i], C[i].i = i;
	std::cout << voronoi_diagram(box, C) << "\n";
	return;
}
