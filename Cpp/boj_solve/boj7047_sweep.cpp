...
int128 abs(int128 x) { if (x < 0) x *= -1; return x; }
...
struct Frac {
	...
	bool operator < (const Frac& o) const {
		...
		int128 div1 = abs(x) / abs(den);
		int128 div2 = abs(o.x) / abs(o.den);
		int128 mod1 = abs(x) % abs(den);
		int128 mod2 = abs(o.x) % abs(o.den);
		if (div1 == div2) {
...
Frac intersection(const Pos& p1, const Pos& p2, const Pos& q1, const Pos& q2) {
	...
	int128 d = gcd(abs(x), abs(det));
	assert(d);
	if (x == 0) det = 1;
	else if (sign(x) * sign(det) < 0 && 0 < x) x *= -1, det *= -1;
	else if (sign(x) * sign(det) > 0) x = abs(x), det = abs(det);
	return { x, det, 0 };
}
void solve() {
	...
	for (int _ = 0; _ < N; _++) {
		...
		tmp.clear();
		for (int i = 0; i < M; i++) {
			const Pos& p0 = P[(i - 1 + M) % M], & p1 = P[i], & p2 = P[(i + 1) % M];
