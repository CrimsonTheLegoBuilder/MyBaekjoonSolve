...
bool connectable2(const std::vector<Pos3D>& P, const Pos3D& a, const Pos3D& b, const ld& th) {//refer to koosaga
	if (zero((a - b).mag())) return 1;
	Pos3D perp = (a / b).unit();
	Pos3D X = a.unit();//X-axis
	Pos3D Y = (perp / a).unit();//Y-axis
...
void solve(const int& tc) {
	...
	for (int i = 0; i < sz; i++) {
		adj[i][i] = 0;
		for (int j = 0; j < i; j++) {
			if (connectable2(pos, pos[i], pos[j], TH))
