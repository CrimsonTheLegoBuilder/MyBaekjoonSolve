...
struct Line {//ax + by = c
	Pos s;
	ll c;
	inline Line(Pos ps = Pos(0, 0), Pos pe = Pos(0, 0)) {
		s = (pe - ps).s();
		c = s * ps;
	}
...
//inline int ccw(const Pos& d1, const Pos& d2, const Pos& d3, const Pos& d4) { ll ret = cross(d1, d2, d3, d4); return !ret ? 0 : ret > 0 ? 1 : -1; }
inline ll inner_check(const Pos& p1, const Pos& p2, const Pos& p3, const Pos& q) { return ccw(p1, p2, q) < 0 && ccw(p2, p3, q) < 0 && ccw(p3, p1, q) < 0; }
inline bool cmp_frac(BigInt p, BigInt q) {
	if (p.lo < 0) p = -p;//p.den < 0
	if (q.lo < 0) q = -q;//q.den < 0
	return mul(p.hi, q.lo) < mul(q.hi, p.lo);
}
inline bool eq_frac(BigInt p, BigInt q) {
	if (p.lo < 0) p = -p;//p.den < 0
	if (q.lo < 0) q = -q;//q.den < 0
	return mul(p.hi, q.lo) == mul(q.hi, p.lo);
}
inline bool above(const Line& l, const Pos& inx, bool rvs = 0) {
	BigInt dist = dot(l.s, inx);
	if (rvs) return cmp_frac(dist, BigInt(l.c, 1));
	return cmp_frac(BigInt(l.c, 1), dist);
}
inline bool on_line(const Line& l, const Pos& inx) {
	BigInt dist = dot(l.s, inx);
	return eq_frac(dist, BigInt(l.c, 1));
}
//inline bool closer(const Pos& p1, const Pos& p2, const Pos& q, const Pos& candi) {
...
//}
inline bool closer(const Pos& p1, const Pos& p2, const Pos& q, const Pos& candi) {
	return std::abs(cross(p1, p2, q)) > std::abs(cross(p1, p2, candi));
}
...
//	return "NIE";
//}
std::string half_plane_intersection_except_x(const int& x = -1) {
	int sz = (x == -1 ? N : x + 1);
	int i = (x == -1 ? 0 : x);
...
std::string query() {
	...
	if (illum == N) return half_plane_intersection_except_x();
