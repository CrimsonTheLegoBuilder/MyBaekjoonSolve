...
int max_y[LEN];
Sint inxs[LEN << 2];//seg_tree
int seg_v[LEN << 2];//seg_tree
int seg_p[LEN << 2];//seg_tree
int add_horizontal(int l, int r, int idx, int s = 0, int e = xi - 1, int i = 1) {
	if (r < s || e < l) return 0;
	if (l <= s && e <= r) {
	...
	int m = s + e >> 1;
	return add_horizontal(l, r, idx, s, m, i << 1) + add_horizontal(l, r, idx, m + 1, r, i << 1 | 1);
}
...
void solve() {
	...
	int tmp;
	tmp = -1;
	for (const int& x : X) if (x != tmp) tmp = CX[xi++] = x;
	tmp = -1;
	for (const int& y : Y) if (y != tmp) tmp = CY[yi++] = y;
	std::sort(H, H + hp);
	std::sort(V, V + vp);
	memset(P, -1, sizeof P);
	...
	for (int y = 0, i = 0, j = 0; y < yi; y++) {
		...
		while (j < hp && idx_bi_search(CY, yi, H[j].y) == y) {
			...
			int cnt = add_horizontal(l_, r_, H[j].i);
			e += cnt * 2 + 1;
			v += cnt + 2;
			j++;
		}
