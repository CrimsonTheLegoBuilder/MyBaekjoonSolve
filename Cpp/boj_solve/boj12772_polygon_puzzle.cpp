...
Polygon rotate(Polygon B, const int& j0, const Polygon& A, const int& i0, const ld& t) {
	int sz = B.size();
...
struct Line {//ax + by = c
	Pos s;
	ll c;
	inline Line(Pos V = Pos(0, 0), ll C = 0) : s(V), c(C) {}
	inline bool operator == (const Line& l) const { return s == l.s && c == l.c; }
...
inline bool two_polygon_equal_check(std::vector<Bound>& V1, std::vector<Bound>& V2) {
	int sz = V1.size();
...
int polygon_cross_check(Polygon A, Polygon B) {
	INNER_CHECK = 0;
	EDGE_IGNORE = 0;
	ai = bi = -1;
...
ld sweep(const Polygon& A, const Polygon& B) {
	for (int i = 0; i < N; i++) {
...
void solve() {
	...
	for (int i = 0; i < N; i++) {
		...
		for (int j = 0; j < M; j++) {
			...
			Polygon B2 = rotate(B, j, A, i, t);
			ret = std::max(ret, sweep(A, B2));
		}
	}
	std::cout << ret << "n";
	return;
