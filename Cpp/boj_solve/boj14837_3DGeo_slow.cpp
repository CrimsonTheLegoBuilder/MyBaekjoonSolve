...
struct Face2 {//refer to BIGINTEGER
	int v[3];
	Face2(int a, int b, int c) { v[0] = a; v[1] = b; v[2] = c; }
...
std::vector<Face2> H;
std::vector<Face2> ConvexHull3D(std::vector<Pos3D>& P) {//refer to BIGINTEGER
	COL = 0; COP = 0;
	int suf = prep(P);
	if (suf == 2) { COL = 1; return {}; };
	if (suf == 3) { COP = 1; return {}; };
	...
	std::vector<Face2> cur;
	cur.push_back(Face2(0, 1, 2));
	cur.push_back(Face2(2, 1, 0));
	for (int i = 3; i < sz; i++) {
		std::vector<Face2> next;
...
int above(const std::vector<Pos3D>& C, const Face2& F, const Pos3D& p) {
	Pos3D nrm = cross(C[F.v[0]], C[F.v[1]], C[F.v[2]]);
	ll ret = nrm * (p - C[F.v[0]]);
	return !ret ? 0 : ret > 0 ? 1 : -1;
}
int inner_check(const std::vector<Pos3D>& C, const std::vector<Face2>& F, const Pos3D& p) {
	bool cop = 0;
	bool out = 0;
	for (const Face2& face : F) {
		int f = above(C, face, p);
		if (f == 0) cop = 1;
		if (f > 0) out = 1;
	};
