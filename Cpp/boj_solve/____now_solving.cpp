...
struct Pos {
	...
	int i;
	bool f;
	Pos(ld x_ = 0, ld y_ = 0, int i_ = 0) : x(x_), y(y_), i(i_) { f = 0; }
...
//Segs VDN[LEN];
Seg S[LEN];
Pos symmetric(const Seg& se, const Pos& p) {
	ld tq = cross(se.s, se.e, p);
	if (zero(tq)) return p;
	ld d = tq / se.dir.mag();
	Pos v = -~se.dir * d;
	return p + v + v;
...
void query() {
	...
	Polygon A, B, C;
	for (int i = 0; i < N; i++) {
		A.push_back(symmetric(S[i], a));
		B.push_back(symmetric(S[i], b));
	}
	A.push_back(a);
	B.push_back(b);
	//std::cout << "FUCK::\n";
	...
	for (Pos& p : A) C.push_back(p);
	for (Pos& p : B) C.push_back(p);
	std::sort(C.begin(), C.end());
	...
	C.erase(unique(C.begin(), C.end()), C.end());
	//std::cout << "C.sz:: " << C.size() << "\n";
	if (sz != C.size()) { std::cout << "0.000000000000\n"; return; }
	sz = C.size();
	for (int i = 0; i < sz; i++) {
		C[i].i = i;
		DT[i].clear();
...
void test() {
	std::cin >> N >> Q;
	Pos s, e;
	for (int i = 0; i < N; i++) std::cin >> s >> e, S[i] = Seg(s, e);
	while (Q--) query();
	return;
}
void solve() {
