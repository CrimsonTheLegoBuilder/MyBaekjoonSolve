...
void solve() {
	...
	std::cin >> N;
	Polygon clip(N);
	for (Pos& p : clip) std::cin >> p;
	std::cin >> M;
	seed.resize(M);
	for (Pos& p : seed) std::cin >> p;
	std::cin >> s >> e;
	bool rvs = e < s;
	for (int i = 0; i < M; i++) {
		Lines HP;
		for (int j = 0; j < M; j++) {
			if (i == j) continue;
			HP.push_back(rot90(L(seed[i], seed[j]), (seed[i] + seed[j]) * .5));
		}
		for (int j = 0; j < N; j++) {
			HP.push_back(L(clip[(j + 1) % N], clip[j]));
		}
		//assert(half_plane_intersection(HP, vd[i]));
		half_plane_intersection(HP, vd[i]);
		Polygon seg = sutherland_hodgman({ s, e }, vd[i]);
		if (!seg.size()) continue;
		else {
			std::sort(seg.begin(), seg.end());
			Pos x = seg[0], y = seg.back();
			root.push_back(Seg(x, y, i));
		}
	}
	std::sort(root.begin(), root.end());
	if (rvs) std::reverse(root.begin(), root.end());
#ifdef DEBUG
	for (Seg& S : root) std::cout << S << "\n";
#endif
	std::cout << root.size() << "\n";
	for (const Seg& S : root) std::cout << S.i + 1 << " ";
	return;
}
int main() { solve(); return 0; }//boj6728 NCPC 2001 H Request for permission
