...
ld trial(Pos p[], int s[]) {  //inner check
	std::vector<Line> HP;
	HP.push_back(L(pr, pu)); HP.push_back(L(pu, pl)); HP.push_back(L(pl, pd)); HP.push_back(L(pd, pr));
	for (int i = 1; i < N; i++) {
		ld dy = p[s[i]].y - p[s[i + 1]].y;
		ld dx = p[s[i + 1]].x - p[s[i]].x;  // -(p[l].x - p[r].x)
		ld c = dy * p[s[i]].x + dx * p[s[i]].y;// -hypot(dy, dx) * 1e-5;
