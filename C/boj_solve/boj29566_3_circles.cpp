...
std::vector<Pos> find_inx(const Circle& c1, const Circle& c2) {
	if (c1.c == c2.c) return {};
	if ((c1.c - c2.c).mag() > (c1.R + c2.R)) return {};
	if ((c1.c - c2.c).mag() < std::abs(c1.R - c2.R)) return {};
	ld A = c1.R * c1.R, B = c2.R * c2.R;
	ld C = (c1.c - c2.c).mag() * (c1.c - c2.c).mag();
	ld cos_t = (A + C - B) / (2 * c1.R * (c1.c - c2.c).mag());
	ld theta = acos(cos_t);
	//std::cout << theta << "\n";
	Pos vec = c2.c - c1.c;
	...
	Pos d1 = { c1.c.x + vx1 * c1.R / (c1.c - c2.c).mag(), c1.c.y + vy1 * c1.R / (c1.c - c2.c).mag() };
	Pos d2 = { c1.c.x + vx2 * c1.R / (c1.c - c2.c).mag(), c1.c.y + vy2 * c1.R / (c1.c - c2.c).mag() };
	if (d1 == d2) return { d1 };
	return { d1, d2 };
}
void solve() {
	...
	if (!A.size()) std::cout << "Impossible\n";
	else if (A.size() == 1) {
		if (z((A[0] - c.c).mag() - c.R)) std::cout << A[0].x << " " << A[0].y << "\n";
		else std::cout << "Impossible\n";
	}
	else {
		if (z((A[0] - c.c).mag() - c.R) && z((A[1] - c.c).mag() - c.R)) std::cout << "Impossible\n";
		if (z((A[0] - c.c).mag() - c.R)) std::cout << A[0].x << " " << A[0].y << "\n";
		else if (z((A[1] - c.c).mag() - c.R)) std::cout << A[1].x << " " << A[1].y << "\n";
		else std::cout << "Impossible\n";
	}
	return;
}
int main() { solve(); return 0; }//boj29566
