...
void get_area() {
	...
	for (int i = 0; i <= N; i++) {
		std::cout << "memo " << memo[i] << "\n";
	}
	return;
}
int inner_bi_check(const Pos& p, Pos H[], int h) {
...
Pos find_tangent(Pos H[], const Pos& p) {
	int i1{ 0 }, i2{ 0 };
	...
	if (ccw1 * ccwN >= 0) {
		...
		while (s + 1< e) {
			k = s + e >> 1;
			Pos p1 = p, cur = H[0], nxt = H[k];
			if (!f) std::swap(p1, cur);
			if (cross(p1, cur, nxt) > 0) s = k;
			else e = k;
		}
		int s1 = 0, e1 = s;
		while (s1 < e1) {
...
ll get_score(const Pos& shoot) {
	int f = inner_bi_check(shoot, Target, N);
	std::cout << f << "\n";
	if (!f) return 0;
	if (f > 0) return memo[N];
	ll score{ 0 };
	Pos tangent = find_tangent(Target, HB);
	std::cout << tangent.x << " " << tangent.y << "\n";
